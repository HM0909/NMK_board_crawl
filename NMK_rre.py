...
def crawling():
    ...
    page_number = soup.find("div",  {"class" : "allPage"})
    page_number_all = page_number.find_all("span")
    page_number_end = page_number_all[1].text
                                
    datas =[]
    
    # 페이지 이동                     
    for i in range(0, int(page_number_end)):
        i = i + 1
        
        board_page_url = "https://www.museum.go.kr/site/main/archive/post/category/category_52?cp=" + str(i) + "&catId=52" 
        
        driver.get(board_page_url)
...
def detail(board_number, board_Type, board_group, board_wirter, link_url): 
    ...
    if detail_main.find("div",  {"class" : "flie-down-area"}):
        attach_body = detail_main.find("div",  {"class" : "flie-down-area"})
        attach = attach_body.find_all("span")
        
        for list in attach:
            attach_all = list.find("a")
            attach_url= attach_all.get("href")                                              # 첨부파일 URL                
            
    if not attach_body.find_all("span"):
        attach_url = " "
